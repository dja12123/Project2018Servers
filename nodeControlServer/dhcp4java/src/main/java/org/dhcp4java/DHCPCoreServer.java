...
public class DHCPCoreServer implements Runnable
{
	...
	public static final Logger logger = Logger.getLogger(DHCPCoreServer.class.getName().toLowerCase());
	 ...
	 *
	 * @param servlet
	 *            the <tt>DHCPServlet</tt> instance processing incoming requests,
	 *            must not be <tt>null</tt>.
	 * @param userProps
	 *            specific properties, overriding file and default properties, may
	 *            be <tt>null</tt>.
	...
	protected void init() throws DHCPServerInitException
	{
		...
		try
		{
			...
			this.serverSocket = new DatagramSocket(67);
			//this.serverSocket.connect(new InetSocketAddress(InetAddress.getByName("0.0.0.0"), 67));
	...
	public static InetSocketAddress parseSocketAddress(String address)
	{
		...
		try
		{
			serverAddress = InetAddress.getByName(serverStr);
		}
		...
		logger.log(Level.INFO, "address: " + serverAddress.getHostAddress());
